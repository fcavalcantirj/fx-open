...
pub fn Runtime(comptime App: type) type {
    return struct {
        ...
        fn switchProvider(
            ...
            const catalog_fetch_result: ?model_catalog.ProviderResult = blk: {
            ...
            if (catalog_fetch_result) |fetched| {
                switch (fetched) {
                    .catalog => |loaded| {
                        catalog = loaded;
                        catalog_loaded = true;
            ...
            const selected_model = if (catalog_entries.len > 0)
                selectCatalogModel(catalog_entries, current_model, preferred_model) orelse unreachable
            else
                model_provider.resolveSwitchModel(&.{}, saved_model, preferred_model) orelse unreachable;
