...
fn reviewOpenAiCompatible(
    ...
    const config_ptr: *const openai_compatible.OpenAiCompatibleConfig = if (raw_context) |ctx|
...
fn sendReview(
    ...
    var result = stream.stream(alloc, .{
...
test "OpenAI-compatible reviewer builds through the compatible stream provider" {
    ...
    try std.testing.expect(std.mem.find(u8, body, "\"model\":\"llama-local\"") != null);
