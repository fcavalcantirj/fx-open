...
const openai_compatible_permission_reviewer = @import("gateway/openai_compatible_permission_reviewer.zig");
...
const App = struct {
    ...
    pub fn permissionReviewerProvider(self: *const App) ?permission_auto_classifier.Provider {
        const provider = self.provider_selection.selection().provider;
        if (provider == .openai) {
            if (comptime !host_profile.tools or host_target.is_wasm) return null;
            return .{
                .context = @ptrCast(@alignCast(@constCast(&self.openai_config))),
                .review_fn = openai_compatible_permission_reviewer.provider.review_fn,
            };
        }
        return self.subagentProviderRoutes()
            .select(provider)
    ...
    pub fn subagentProviderRoutes(self: *const App) subagent_agent_adapter.ProviderRoutes {
        return .{
            .gateway = .{
...
fn fullEntryConfig() app_entry_runtime.Config {
    return .{
        ...
        .openai_permission_reviewer_provider = openai_compatible_permission_reviewer.provider,
...
test {
    ...
    _ = @import("gateway/openai_compatible_permission_reviewer.zig");
